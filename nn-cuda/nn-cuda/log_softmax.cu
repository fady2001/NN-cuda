 ...
 * @param out: output tensor of shape (N, C) where N is the batch size (number
 * of rows) and C (number of columns) is the number of classes
 * @param N: number of rows
 * @param C: number of columns
 */
template <class T> void log_softmax_cpu(T *in, T *out, int N, int C) {
...
int main() {
  ...
  // make the input less uniformly random: Otherwise, all probabilities will be
  // basically zero, and the tests are not actually meaningful.
  ...
  for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++) {
    ...
    float elapsed_time = benchmark_kernel(repeat_times, run_kernel1<float>,
                                          d_inp, d_out, N, C, block_sizes[j]);
