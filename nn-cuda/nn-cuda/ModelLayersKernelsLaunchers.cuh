...
public:
	template<class T>
	static void linear_layer(T* X, T* W, T* bias, T* y, int B, int N, int M, int sqrt_block_size)
	{
	...
	template<class T>
	static void run_relu_kernel(T* input, T* output, int B, int N, int sqrt_block_size)
	{
		dim3 block(sqrt_block_size, sqrt_block_size);
	...
	template <class T>
	static void run_softmax_kernel(const T* input, T* output, int N, int C, int block_size)
	{
	...
	template <class T>
	static void run_cross_entropy_kernel(T* losses, const T* probs, const int* targets, int N, int C, const int block_size)
	{
	...
	template <class T>
	static void run_array_sum_kernel3(T* d_a, T* d_result, int size, int block_size)
	{
		// (dividend + divisor - 1) / divisor
