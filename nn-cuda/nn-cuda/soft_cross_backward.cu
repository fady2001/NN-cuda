 ...
 */
template <class T>
void crossentropy_softmax_backward_cpu(T *down_grads, const T *log_softmax,
  ...
  if (reduction == MEAN) {
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < C; j++) {
        down_grads[j + i * C] =
            (exp(log_softmax[j + i * C]) - ((j == targets[i]) ? 1.0f : 0.0f)) /
            N;
      }
    }
  } else {
...
// kernel launcher
template <class T>
void run_crossentropy_softmax_backward(T *down_grads, T *probs, uint *targets,
...
int main() {
...
#if TEST_PYTORTH
  write_npy("cross-entropy-backward\\down_grads.npy", down_grads, 2,
            new unsigned long[2]{N, C});
  write_npy("cross-entropy-backward\\h_targets.npy", h_targets, 1,
            new unsigned long[1]{N});
  write_npy("cross-entropy-backward\\h_probs.npy", h_probs, 2,
            new unsigned long[2]{N, C});
#endif
  ...
  measureExecutionTime(crossentropy_softmax_backward_cpu<float>, down_grads,
                       h_probs, h_targets, N, C, MEAN);

#if TEST_PYTORTH
  write_npy("cross-entropy-backward\\h_dlogits_after.npy", down_grads, 2,
            new unsigned long[2]{N, C});
#endif
  ...
  cudaCheck(cudaMemcpy(d_down_grads, down_grads, N * C * sizeof(float),
                       cudaMemcpyHostToDevice));
  cudaCheck(cudaMemcpy(d_probs, h_probs, N * C * sizeof(float),
                       cudaMemcpyHostToDevice));
  ...
  for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++) {
    ...
    run_crossentropy_softmax_backward(d_down_grads, d_probs, d_targets, N, C,
                                      block_size, REDUCTION::MEAN);
  ...
  for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++) {
    ...
    float elapsed_time = benchmark_kernel(
        repeat_times, run_crossentropy_softmax_backward<float>, d_down_grads,
        d_probs, d_targets, N, C, block_sizes[j], REDUCTION::MEAN);
