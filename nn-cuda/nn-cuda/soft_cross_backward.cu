...
//			for (int j = 0; j < C; j++)
//			{
//				down_grads[j + i * C] = (exp(probs[j + i * C]) - ((j == targets[i]) ? 1.0f : 0.0f)) / N;
//			}
//		}
//	}
//	else
...
//			for (int j = 0; j < C; j++)
//			{
//				down_grads[j + i * C] = exp(probs[j + i * C]) - ((j == targets[i]) ? 1.0f : 0.0f);
//			}
//		}
//	}
//}
...
//		run_crossentropy_softmax_backward(d_down_grads, d_probs, d_targets, N, C, block_size,REDUCTION::MEAN);
...
//		float elapsed_time = benchmark_kernel(repeat_times, run_crossentropy_softmax_backward<float>, d_down_grads, d_probs, d_targets, N, C, block_sizes[j], REDUCTION::MEAN);
//
