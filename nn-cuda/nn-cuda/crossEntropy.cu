...
//  */
//  template<class T>
//  void cross_entropy_cpu(T* losses,T* input, uint* targets,int N, int C) {
...
//  template<class T>
//  __global__ void cross_entropy_kernel(T* losses, T* input, uint* targets, int N, int C) {
...
//  template <class T>
//  void run_cross_entropy_kernel(T *losses, T *probs, uint *targets, int N, int C, const int block_size)
//  {
...
//    write_npy<int>("cross-entropy-layer\\h_targets.npy", h_targets, 1, new unsigned long[1]{N});
//  #endif
...
//  #if TEST_PYTORTH
//   write_npy<float>("cross-entropy-layer\\h_losses.npy", h_losses, 1, new unsigned long[1]{N});
//  #endif
...
//  		run_cross_entropy_kernel(d_losses, d_predictions, d_targets, N, C, block_sizes[j]);
...
//  		float elapsed_time = benchmark_kernel(repeat_times, run_cross_entropy_kernel<float>, d_losses, d_predictions, d_targets, N, C, block_sizes[j]);
