...
//* @param input: input tensor of shape (N, C) where N is the batch size (number
...
output:
  losses is(N) of the individual losses for each batch
 ...
 __global__ void nll_loss_kernel(T* losses, T* input, uint* targets, int N, int C) {
  ...
  template <class T>
  void run_nll_loss_kernel(T *losses, T *probs, uint *targets, int N, int C,
  ...
  int main() {
...
#if TEST_PYTORTH
    write_npy<float>("nll-loss-layer\\h_predictions.npy", h_predictions, 2,
                     new unsigned long[2]{N, C});
    write_npy<uint>("nll-loss-layer\\h_targets.npy", h_targets, 1,
                    new unsigned long[1]{N});
#endif

    move to GPU float *d_losses;
    float *d_predictions;
    uint *d_targets;
    ...
    cudaCheck(cudaMemcpy(d_predictions, h_predictions, N * C * sizeof(float),
...
#if TEST_PYTORTH
    write_npy<float>("nll-loss-layer\\h_losses.npy", h_losses, 1,
                     new unsigned long[1]{N});
#endif

    run the kernel int block_sizes[] = {32, 64, 128, 256, 512, 1024};
    first check the correctness of the
        kernel for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++) {
      int block_size = block_sizes[j];
      printf("Checking block size %d.\n", block_size);
      run_nll_loss_kernel(d_losses, d_predictions, d_targets, N, C,
                          block_sizes[j]);
    ...
    for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++) {
      ...
      float elapsed_time =
          benchmark_kernel(repeat_times, run_nll_loss_kernel<float>, d_losses,
                           d_predictions, d_targets, N, C, block_sizes[j]);

      printf("block_size %4d | time %.4f ms | per token %.2f µs\n", block_size,
             elapsed_time, elapsed_time * 1'000 / (N * C));
    }

    free memory free(h_losses);
    free(h_predictions);
    free(h_targets);
