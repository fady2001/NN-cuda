...
auto reduce_type = REDUCTION::MEAN;
...
void train_step(ModelMemoryHandler& model, float* inp, uint* target, uint B, uint input_dim, uint H1, uint C)
{
	...
	// run the model
	ModelLayers::linear_layer_forward_cpu(
		inp, model.GetParams().ln1w, model.GetParams().ln1b,
		model.GetActivations().ln1, B, input_dim, H1);
	ModelLayers::relu_forward_cpu(model.GetActivations().ln1,
		model.GetActivations().a1, B, H1);
	ModelLayers::linear_layer_forward_cpu(
		model.GetActivations().a1, model.GetParams().ln2w,
		model.GetParams().ln2b, model.GetActivations().ln2, B, H1, C);
	ModelLayers::log_softmax_cpu(
		model.GetActivations().ln2, model.GetActivations().sm, B, C);
	ModelLayers::cross_entropy_cpu(model.GetActivations().loss, model.GetActivations().sm, target, B, C);
	ModelLayers::reduce_cpu(model.GetActivations().loss,
		model.GetActivations().reduced_loss, B,
		reduce_type);
	...
	// backpropagation
	ModelLayers::crossentropy_softmax_backward_cpu(
		model.GetDownstreamGradients().dsm, model.GetActivations().sm,
		target, B, C, reduce_type);
	...
	ModelLayers::relu_backward_cpu(
		model.GetActivations().ln1, model.GetDownstreamGradients().dln2,
	...
	// Magic optimizer
	ModelLayers::SGD_cpu(model.GetParamsMemory(),
		model.GetGradientsMemory(),
