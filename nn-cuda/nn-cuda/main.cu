...
int main() {
  ...
  cudaCheck(cudaMemcpy(dsm, d_model.GetDownstreamGradients().dsm,
                       B * C * sizeof(float), cudaMemcpyDeviceToHost));
#if TEST_PYTORTH
...
#if TEST_PYTORTH
  save_2d("updated_ln1w", d_model.GetParams().ln1w, H1, input_dim);
  save_1d("updated_ln1b", d_model.GetParams().ln1b, H1);
  save_2d("updated_ln2w", d_model.GetParams().ln2w, C, H1);
  save_1d("updated_ln2b", d_model.GetParams().ln2b, C);
#endif
  return 0;
}
