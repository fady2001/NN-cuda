...
void SGD_cpu(float *params_memory, const float *grads_memory,
...
void SGD_run_kernel(float *params_memory, const float *grads_memory,
                    long num_parameters, float learning_rate = 1e-3,
  ...
  SGD_kernel<<<num_blocks, block_size>>>(
      params_memory, grads_memory, num_parameters, learning_rate, weight_decay);
}
...
int main() {
  ...
  float *params_memory;
  float *grads_memory;
  const unsigned long num_parameters = 10000;
...
#if TEST_PYTORTH
  write_npy<float>("SGD-optimizer\\params_memory.npy", params_memory, 1,
                   new unsigned long[1]{num_parameters});
  write_npy("SGD-optimizer\\grads_memory.npy", grads_memory, 1,
            new unsigned long[1]{num_parameters});
#endif
  ...
  cudaCheck(cudaMemcpy(d_grads_memory, grads_memory,
                       num_parameters * sizeof(float), cudaMemcpyHostToDevice));
  cudaCheck(cudaMemcpy(d_params_memory, params_memory,
                       num_parameters * sizeof(float), cudaMemcpyHostToDevice));

  // run cpu
  measureExecutionTime(SGD_cpu, params_memory, grads_memory, num_parameters,
                       1e-3, 0.0);

#if TEST_PYTORTH
  write_npy("SGD-optimizer\\updated_params_memory.npy", params_memory, 1,
            new unsigned long[1]{num_parameters});
#endif
  ...
  for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++) {
    ...
    //    validate_result(d_params_memory, params_memory, "out", num_parameters,
    //                    1e-4f);
  }
  ...
  for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++) {
    ...
    float elapsed_time =
        benchmark_kernel(repeat_times, SGD_run_kernel, d_params_memory,
                         d_grads_memory, num_parameters, 1e-3, 0.0, block_size);
    printf("block_size %4d | time %.4f ms | per token %.2f µs\n", block_size,
           elapsed_time, elapsed_time * 1'000 / (num_parameters));
  }
