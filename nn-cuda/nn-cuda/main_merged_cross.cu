...
//KernelsLaunchers::run_relu_kernel(
...
//KernelsLaunchers::run_cross_entropy_loss_kernel(d_model.get_activations().ln2, d_target, d_model.get_activations().sm,d_model.get_activations().loss, B, C, 32);
//save_2d("all-model\\sm.npy", d_model.GetActivations().sm, B, C);
//save_1d("all-model\\loss.npy", d_model.GetActivations().loss, B);
//KernelsLaunchers::run_reduce_kernel3(d_model.GetActivations().loss, d_model.GetActivations().reduced_loss, B,REDUCTION::SUM,32);
//
//// cuda synchronize();
...
//save_2d("all-model\\dln1.npy", d_model.GetDownstreamGradients().dln1, B, input_dim);
//save_2d("all-model\\ln1w_grad.npy", d_model.GetGradients().ln1w_grad, H1, input_dim);
...
//
//#if TEST_PYTORTH
//save_2d("all-model\\updated_ln1w.npy", d_model.GetParams().ln1w, H1, input_dim);
