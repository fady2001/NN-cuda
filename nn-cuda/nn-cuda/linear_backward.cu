 ...
 *
 */
//__global__ void mat_mul_naive(const float *A, const float *B, float *C, uint
// N,
...
int main() {
  srand(0);
  const size_t B = 2048, N = 2048, M = 2048;
...
// write arrays to npy files if you want to test with torch
  ...
  //  run_linear_backward_cpu(inp, weight, up_grad, dldw_cpu, dldb_cpu,
  //  dldx_cpu, B,
  //                          N, M);
  // move to GPU
  float *d_inp;
  ...
  for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++) {
    ...
    float elapsed_time =
        benchmark_kernel(repeat_times, runBackward, d_inp, d_weight, d_up_grad,
                         d_dLdw, d_dLdb, d_dLdx, B, N, M, block_sizes[j]);

    float tflops1 = (float)B * N * M * 2 / elapsed_time * 1e3f / 1e12f;
    printf("block_size %4d | time %.4f ms | per token %.2f µs\n", block_size,
           elapsed_time, elapsed_time * 1'000 / (B * N));
    //    printf("Block size %d: %f TFLOPS\n", block_size, tflops1);
  }
#if TEST_PYTORTH
