...
// kernel 1
template <class T>
__global__ void reduce_kernel1(T *out, const T *in, int N,
                               REDUCTION reduction) {
...
// kernel 2
template <class T>
__global__ void reduce_kernel2(T *d_a, T *d_result, int size,
                               REDUCTION reduction) {
...
template <class T>
void run_kernel1(T *d_inp, T *d_result, int N, int block_size,
                 REDUCTION reduction) {
  reduce_kernel1<<<(N + block_size - 1) / block_size, block_size>>>(
      d_inp, d_result, N, reduction);
}

template <class T>
void run_kernel2(T *d_inp, T *d_result, int N, int block_size,
                 REDUCTION reduction) {
  reduce_kernel2<<<1, block_size, block_size * sizeof(T)>>>(d_inp, d_result, N,
                                                            reduction);
}

template <class T>
void run_kernel3(T *d_inp, T *d_result, int N, int block_size,
                 REDUCTION reduction) {
  reduce_kernel3<<<1, block_size, block_size * sizeof(T)>>>(d_inp, d_result, N,
                                                            reduction);
}

int main() {
  // Placeholder for actual test implementation
  // This should include allocation of device memory, kernel launches, and
  // copying results back to host Example usage:
  const int N = 10000;
  ...
  for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++) {
    ...
    // float elapsed_time1 = benchmark_kernel(repeat_times,run_kernel1<float>,
    // d_in1, d_out1, N, block_size, reduction);
    float elapsed_time1 =
        benchmark_kernel(repeat_times, run_kernel1<float>, d_in2, d_out2, N,
                         block_size, reduction);
    float elapsed_time2 =
        benchmark_kernel(repeat_times, run_kernel2<float>, d_in2, d_out2, N,
                         block_size, reduction);
    float elapsed_time3 =
        benchmark_kernel(repeat_times, run_kernel3<float>, d_in3, d_out3, N,
                         block_size, reduction);
