...
public:
	...
	static void run_nll_loss_kernel(float* losses, const float* probs,
		const uint* targets, uint N, uint C,
		const uint block_size) {
		const int grid_size = (N + block_size - 1) / block_size;
		nll_loss_kernel << <grid_size, block_size >> > (losses, probs, targets, N,
			C);
		cudaCheck(cudaGetLastError());
	}

	template <class T>
	static void run_cross_entropy_loss_kernel(T* in, uint* targets, T* softmaxed, T* losses, int N, int C, int block_size)
	{
